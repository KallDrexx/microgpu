...
static const Mgpu_Allocator basicAllocator = {
        .FastMemAllocateFn = malloc,
        .FastMemFreeFn = free,
        .SlowMemAllocateFn = malloc,
        .SlowMemFreeFn = free,
};
...
void handleResponse(Mgpu_Response *response) {
    switch (response->type) {
        case Mgpu_Response_Status:
            ...
            SDL_Log("Api version: %u\n", response->status.apiVersionId);
            SDL_Log("\n");
            break;
...
void wait_for_init_op() {
    ...
    if (!mgpu_texture_define(textureManager, &frameBufferSpecs, operation.initialize.frameBufferScale)) {
        SDL_Log("Initialization failed: framebuffer texture could not be defined");
        abort();
    }
